...
void program_init()
{
	...
	traj.add_constraint(0.0, 0, Eigen::VectorXd::Zero(num_vars));
	traj.add_constraint(0.0, 1, Eigen::VectorXd::Zero(num_vars));
	traj.add_constraint(0.0, 2, Eigen::VectorXd::Zero(num_vars));
	
	Eigen::VectorXd final_pos(num_vars);
	final_pos << 6.0, 3.0;
	traj.add_constraint(tf, 0, final_pos);
	traj.add_constraint(tf, 1, Eigen::VectorXd::Zero(num_vars));
	traj.add_constraint(tf, 2, Eigen::VectorXd::Zero(num_vars));
	
	traj.generate();

	const double delta_t = 0.1;
	int N = (int)(tf / delta_t);
	Eigen::MatrixXd path(N, num_vars);
	for (int i = 0; i < N; ++i)
	{
		double t = 0.0 + delta_t * i;
		path.row(i) = traj.eval(t);
	}
